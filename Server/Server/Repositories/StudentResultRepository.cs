...
namespace Server.Repositories
{
    public class StudentResultRepository : GenericRepository<StudentResult>, IStudentResultRepository
    {
        ...
        {
            StudentResult student = await _dbContext.StudentResult.Include(x => x.Student).ThenInclude(x => x.Exams).ThenInclude(x => x.Subject).FirstOrDefaultAsync(x => x.StudentId == studentId && x.ExamId == examId);
            return student;
        }
    }
}
